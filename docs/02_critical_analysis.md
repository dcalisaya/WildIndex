...
| :--- | :--- | :--- |
| **"Vendor Lock-in" con NAS** | Medio | Si se cambia de marca de NAS, la búsqueda falla. **Solución:** Usar estándares estrictos (XMP/IPTC) que sean legibles por cualquier software (Adobe Bridge, Lightroom, DigiKam), no solo Synology/QNAP. |
...
1.  **Dockerización:** Crear imagen `wildindex:base` con PyTorch y drivers pre-compilados (ahorra horas de install).
2.  **Pipeline V1 (Solo Detección):** MegaDetector -> JSON -> XMP Injection.
3.  **Validación de Metadatos:** Confirmar que los gestores de fotos leen los tags XMP inyectados y las regiones.
...
1.  **"El Eliminador de Basura":**
    ...
    *   **Valor:** Se recupera espacio en disco y limpia la galería inmediatamente.
...
*   **Recomendación Final:** No construyas una nueva interfaz de usuario (UI) todavía. Tu "UI" es el explorador de archivos y el gestor de fotos nativo. Haz que esos funcionen perfecto con tus metadatos. Esa es la victoria rápida y escalable.
